...
/********************************************************M******************************/		
	for (int i = 0; i < active_ghosts; ++i)
		{
		        if ( board -> is_mine ( player -> getX (), player -> getY () ))
		         { mine_active = true;}
		           
			if ( mine_active ==true && board -> is_mine ( ghosts [i] -> getX (), ghosts [i] -> getY () ))
				{
			//		if ( ghosts [i] -> is_frightened () )
					//	{
						board -> eat_dot ( ghosts [i] -> getX (), ghosts [i] -> getY () );
							ghosts [i] -> set_frightened_mode ( false );
...
/*void CGhostManager::check_collisions_mine ( CBoard * board )
	{
		for (int i = 0; i < active_ghosts; ++i)
		{
			if ( board -> is_mine ( ghosts [i] -> getX (), ghosts [i] -> getY () ))
				{
					if ( ghosts [i] -> is_frightened () )
						{
							ghosts [i] -> set_frightened_mode ( false );
					...
					else board -> eat_dot ( ghosts [i] -> getX (), ghosts [i] -> getY () );
				}
		}
