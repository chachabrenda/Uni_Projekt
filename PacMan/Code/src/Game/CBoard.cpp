...
bool CBoard::is_wall   ( int x, int y ) const   
	{
		if ( board -> at (x, y) == 'x' || board -> at (x, y) == '#' || board -> at (x, y) == '/' )
			return true;
		return false;
	}
/******************************************M***********************************************************/
	
bool CBoard::is_wall_wall   ( int x, int y ) const   /*M*/
	{
...
/******************************************M*******************************/
bool CBoard::is_door ( int x, int y ) const
	{
		if ( board -> at (x, y) == '/')
			return true;
		return false;
...
/*M*/
bool CBoard::is_mine ( int x, int y ) const
	{
		if ( board -> at (x, y) == '+')
			return true;
		return false;
		...
		for (int i = 0; i < get_height (); ++i)
		{
			for (int j = 0; j < get_width (); ++j)
			{
				if ( is_wall_hash (j, i) )
					...
					wind -> print ( (int) '+', j, i, COLOR_PAIR(PAIR_RED_BLACK));
				else if ( is_door 	( j, i ))
					wind -> print ( (int) '/', j, i, COLOR_PAIR(PAIR_BLUE_BLUE));
